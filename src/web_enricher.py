...
def enrich_contact_info(playlist_name,curator_name='',playlist_url=''):
    ...
    conf=(35 if best['email'] else 0)+(20 if best['instagram'] else 0)+(18 if best['submission_page'] else 0)+(10 if (best['website'] or best['link_hub']) else 0)+(5 if spot.get('title') else 0)+int(top_method*.25)
    return {'playlist_name_found':playlist_name,'curator_name_found':curator_name,'spotify_description':spot.get('description',''),'thumbnail_url':spot.get('thumbnail_url',''),'email':best['email'],'instagram':best['instagram'],'website':best['website'] or best['link_hub'],'submission_page':best['submission_page'],'link_hub':best['link_hub'],'contact_methods':methods,'confidence_score':min(100,conf)}
